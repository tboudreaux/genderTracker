    ...
    <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
      <form>
        ...
        <div v-if="this.$store.state.isGuest" class="mb-4">
          ...
          <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="username" type="text" placeholder="Enter name" ref="guestName">
        </div>
...
export default {
  ...
  methods: {
    ...
    submitData() {
      ...
      // const guestName = this.$store.state.isGuest ? this.$refs.guestName.value : 'None';
      const payload = {
        'gender': gender,
        'pronouns': pronouns,
        'genderDescription': genderDescription,
        'dayDescription': dayDs,
        'name': this.$store.state.isGuest ? this.$refs.guestName.value : null,
      }
